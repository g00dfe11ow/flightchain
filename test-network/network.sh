...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/applicant/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Applicant Identities"

    createApplicant

    infoln "Creating Approver Identities"
  ...
  infoln "Generating CCP files for Applicant, Approver and User"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/applicant/msp organizations/fabric-ca/applicant/tls-cert.pem organizations/fabric-ca/applicant/ca-cert.pem organizations/fabric-ca/applicant/IssuerPublicKey organizations/fabric-ca/applicant/IssuerRevocationPublicKey organizations/fabric-ca/applicant/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/approver/msp organizations/fabric-ca/approver/tls-cert.pem organizations/fabric-ca/approver/ca-cert.pem organizations/fabric-ca/approver/IssuerPublicKey organizations/fabric-ca/approver/IssuerRevocationPublicKey organizations/fabric-ca/approver/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=docker/docker-compose-flightchain.yaml
