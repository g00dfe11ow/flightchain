...
type Plan struct {
	...
	UnitOrIndividual string   `json:"UnitOrIndividual"` // 执行单位或个人
...
func (s *SmartContract) CreatePlan(ctx contractapi.TransactionContextInterface, id string, unitorindividual string, contact string, aircraft Aircraft) error {
...
// 用参数来修改已有的飞行计划
func (s *SmartContract) ModifyPlan(ctx contractapi.TransactionContextInterface, id string, unitorindividual string, contact string, aircraft Aircraft) error {
...
// 将某用户从ReadSet中删除
func (s *SmartContract) RemoveReader(ctx contractapi.TransactionContextInterface, id string, newReader string) error {
	// 只有ApplicantMSP能修改飞行计划的ReadSet
	mspID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return err
	...
	if _, res := InString(plan.ReadSet, newReader); !res {
		...
		newset, err := Remove(plan.ReadSet, newReader)
		if err != nil {
			return err
		}
		plan.ReadSet = newset
